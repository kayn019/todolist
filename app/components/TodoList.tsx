...
const TodoList = () => {
  ...
  useEffect(() => {
    const storedTodos = JSON.parse(localStorage.getItem(`todos-${currentProjectIndex}`) || '[]');
  ...
  return (
    <div  className="grid-cols-2  sm:flex w-full h-screen">
      <div className="bg-black w-full flex flex-col sm:w-2/6 md:w-3/12  text-white p-5 shadow-2xl">
        ...
        <div className='flex flex-col '>
            ...
            <div key={'div'+index} className=" flex justify-between ">
                <button key={'proj'+index} className='hover:bg-slate-300' onClick={() => handleProjectChange(project)}>
                    {project}
                </button>
                    <button key={'del'+index} onClick={() => handleDeleteProject(project)}>
        ...
        <button className="self-center bg-green-300 hover:bg-green-400 focus:bg-green-500 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center my-3 h-auto" onClick={toggleModal}>
      ...
      <input
        ...
        className='h-10'
      />
      <button className="bg-green-300 hover:bg-green-400 focus:bg-green-500 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center " onClick={handleAddTodo}>
        <h5 className="mr-2">Add Todo</h5>
        <IoAddCircleOutline />
        </button>
      <ul className='mt-10 w-full'>
        {todos && todos.map((todo, index) => (
          <li key={index} className='flex flex-row justify-between content-center mb-2 '>
            ...
            <button className='  bg-red-300 hover:bg-red-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center ' onClick={() => handleDeleteTodo(index)}>Delete</button>
          </li>
        ))}
      </ul>
      </div>
